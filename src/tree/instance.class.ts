...
import neo4j, { Driver, session, Session, Transaction } from 'neo4j-driver'
...
export default class Instance implements INode {
    ...
    async getTreeItem() {
        ...
        if ( this.host.endsWith('.neo4j.io') ) {
            ...
            try {
                const res = await session.run('CALL dbms.components')
                edition = ` (${res.records[0].get('edition')})`
                const versions = res.records[0].get('versions')
                if ( versions[0].includes('aura') ) icon = 'database-aura.svg'
            }
            catch (e) {}
        ...
        return {
            id: this.id,
            label: `${this.name || this.id}${edition}`,
            collapsibleState: vscode.TreeItemCollapsibleState.Expanded,
            contextValue: "connection",
            iconPath: path.join(__filename, '..', '..', '..', 'images', icon)
        }
    }
    ...
    async getChildren(): Promise<INode[]> {
        ...
        if ( this.error ) {
            return Promise.resolve([ new Label(this.error.message, 'error', path.join(__filename, '..', '..', '..', 'images', 'alert.svg')) ])
        }
    ...
    getUsers(tx: Transaction): Promise<INode> {
        return tx.run('SHOW USERS')
            .then(res => {
                ...
                return new Label('Users', 'users', path.join(__filename, '..', '..', '..', 'images', 'users.svg'), children)
            })
    ...
    getRoles(tx: Transaction): Promise<INode> {
        return tx.run('SHOW ALL ROLES WITH USERS')
            .then(res => {
                ...
                return new Label('Roles', 'roles', path.join(__filename, '..', '..', '..', 'images', 'roles.svg'), children)
            })
